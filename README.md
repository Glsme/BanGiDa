...
- 프로젝트 내에서 `String`, 또는 `Int`값들을 raw하게 모두 직접 넣어줬다. 하지만 추후 유지•보수 시 비용이 많이 발생하는 것을 느꼈다. 따라서 raw값들을 쉽게 관리해야하는 필요성을 인지하였고, 이후 새싹 스터디 프로젝트에서 raw값을 열거형으로 관리하는 방식을 구조 설계에 반영했다.
</br><br/>
</br><br/>
